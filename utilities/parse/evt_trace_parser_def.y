...
%type<ARRAY[TUPLE[name: STRING; args: ARRAY[EVT_ARG]]]>	evt_trace 
%type<ARRAY[EVT_ARG]> 						evt_arg_list
%type<EVT_ARG>								evt_arg
%type<PRIMITIVE_ARG>						primitive_arg
%type<COMPOSITE_ARG>						composite_arg
%type<ARRAY[PRIMITIVE_ARG]>					primitive_arg_list
%type<ARRAY[PRIMITIVE_ARG]>					primitive_simple_arg_list 	-- for tuple expressions
%type<PRIMITIVE_ARG>						primitive_simple_arg		-- for tuple expressions
...
evt_trace :
	-- Empty rule, create an empty array
		{ 
		  create {ARRAY[TUPLE[name: STRING; args: ARRAY[EVT_ARG]]]} $$.make_empty
		}
...
primitive_simple_arg :
      TK_TRUE  					
	  { 
		create {BOOL_ARG} $$.make(true) 
	  }
    | TK_FALSE 					
	  { 
		create {BOOL_ARG} $$.make(false) 
	  }
 	| TK_TRUE_LOWER  					
	  { 
		create {BOOL_ARG} $$.make(true) 
	...
	| CHAR_LIT
	  {
		create {CHAR_ARG} $$.make(last_character)
	  }
	| STR_LIT
	  { 
		create {STR_ARG}  $$.make(last_string_value) 
	  }
	| NUMBER  					
	  { 
		create {INT_ARG}  $$.make(last_integer_value)  
	  }
	| TK_MINUS NUMBER
	  {
	 	create {INT_ARG}  $$.make(-last_integer_value)  
	...
	| REAL						
	  { 
		create {REAL_ARG} $$.make(last_real_value) 
	...
	| IDENT
	  {
		if enum_items.has_key ($1) then
			create {ENUM_INT_ARG} $$.make (enum_items[$1])
			$$.set_src_out ($1)
		else
			report_error ("Unknown identifier " + $1)
			create {ENUM_INT_ARG} $$.make (-1) -- just to make void safety work!
		end
	  }
	;
