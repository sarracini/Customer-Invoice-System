...
%type<HASH_TABLE[NAMED_PARAM_TYPE, STRING]>		type_decl_list
...
type_decl_list : 
	-- Empty type declarations, create an empty hash table
		{
		  create {HASH_TABLE[NAMED_PARAM_TYPE, STRING]} $$.make(10)
		  $$.compare_objects
	   	}
	|	type_decl_list TK_TYPE IDENT TK_EQ evt_param_type
		{
		  ...
		  $$ := $1
		  $$.extend (create {NAMED_PARAM_TYPE}.make ($3, $5), $3)
		  named_types := $$
		}
		;
...
evt_param_decl :
      ...
      {	
		if attached {TUPLE_PARAM} $3 as tup then
		  check_tuple_member_name_clash ($1, tup)
		end
		
		if attached {NAMED_PARAM_TYPE} $3 as nt and then 
		     attached {TUPLE_PARAM} nt.type as tup 
		then
		  check_tuple_member_name_clash ($1, tup)
		end
	...
	| IDENT                
	  { 
		...
		if NOT named_types.has_key($1) then
			...
			if attached {PARAM_TYPE} named_types[$1] as param_type then
...
primitive_simple_param_type :
   	...
   	| TK_INTEGER  									{ create {INT_PARAM}      $$                         }
	|          NUMBER TK_UPTO          NUMBER		{ create {INTERVAL_PARAM} $$.make (     $1,      $3)
													  check_interval_spec ($$) 
													}
	| TK_MINUS NUMBER TK_UPTO          NUMBER	    { create {INTERVAL_PARAM} $$.make (-1 * $2,      $4) 
													  check_interval_spec ($$) 
													}
	|          NUMBER TK_UPTO TK_MINUS NUMBER		{ create {INTERVAL_PARAM} $$.make (     $1, -1 * $4) 
													  check_interval_spec ($$) 
													}
	| TK_MINUS NUMBER TK_UPTO TK_MINUS NUMBER       { create {INTERVAL_PARAM} $$.make (-1 * $2, -1 * $5) 
													  check_interval_spec ($$) 
													}
    | TK_REAL 										{ create {REAL_PARAM}     $$                         }
	| TK_VALUE										{ create {VALUE_PARAM}    $$                  		 }
	| TK_BOOL 										{ create {BOOL_PARAM}     $$                         }
	| TK_BOOLEAN									{ create {BOOL_PARAM}     $$                         }
	| TK_CHAR 										{ create {CHAR_PARAM}     $$                         }
	| TK_CHARACTER 									{ create {CHAR_PARAM}     $$                         }
	| TK_STR  										{ create {STR_PARAM}      $$                         }
	| TK_LCBRAC enum_item_list TK_RCBRAC			{ create {ENUM_PARAM}     $$.make ($2) }		
	| IDENT                
	  { 
	    -- check that type names are unique 
		if NOT named_types.has_key($1) then
		    ...
		    if attached {NAMED_PARAM_TYPE} named_types[$1] as nt then
		      if attached {PRIMITIVE_PARAM_TYPE} nt.type then
...
feature -- access  
	...
	named_types : HASH_TABLE[NAMED_PARAM_TYPE, STRING]
...
feature -- checks
 	check_interval_spec (primitive_param_type: PRIMITIVE_PARAM_TYPE)
		do
		  if attached {INTERVAL_PARAM} primitive_param_type as interval then
	...
	check_tuple_member_name_clash (param_name: STRING; tup: TUPLE_PARAM)
		do
	...
	check_tuple_member_name_unique (prim_param_type: PRIMITIVE_PARAM_TYPE)
		do
		  if attached {TUPLE_PARAM} prim_param_type as tup then
			from  
