...
feature {NONE} -- mode handling procedures
	...
	handle_batch_file_mode(input_file: FILE_UTILITY; output_file: PLAIN_TEXT_FILE)
	...
	handle_window_mode(input_file: FILE_UTILITY)
		...
		local
	    	...
	    	num_cmds: INTEGER
		do
		  remove_comment_lines (input_file)
...
feature {NONE} -- auxiliary procedures
	...
	remove_comment_lines (f: FILE_UTILITY)
		...
		do
			...
			then
			  if start_of_window > 1 then
			  	...
			  	create sys.make
			  	io.put_string (sys.output.initial_state)
			  end
		    -- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	exec_without_log (input_cmds: STRING)
		...
		do
			create sys.make
			create exec_log.make_empty
		    sys.execute_without_log (input_cmds, is_init)
